"""
engines/timesheet_engine.py
=============================
Calculation logic for the TIMESHEET GENERATOR section only.
Nothing in here is shared with the Daily Check-Up section — each section
has its own engine file on purpose, so changing one can never break the
other.
...
def classify_files(file_dict):
    """
    file_dict: {filename: file-like object}. Returns (system_file, emp_file,
    vac_file) as the same file-like objects, seeked back to position 0.
    """
